...
def load_mesh(path, abg=[50, 15, -90], get3d=True, usepickle=True, usejoblib=False):
    ...
    if usepickle and (os.path.isfile(pickle_file)):
        logging.info("The usepickle == True)")
        logging.info(
            "The pickle file for python {} exists.".format(str(python_version))
        )
        logging.info("The mesh will be loaded from {}".format(pickle_file))
        ...
        logging.info("The usepickle == True")
        logging.info(
            "The pickle file for python {} DO NOT exists".format(str(python_version))
        )
        logging.info("The mesh will be saved to {}".format(pickle_file))

        mesh = fesom_mesh(path=path, abg=abg, get3d=get3d)
        logging.info("Use pickle to save the mesh information")
        logging.info("Save mesh to binary format")
    ...
    if (usejoblib == True) and (os.path.isfile(joblib_file)):
        logging.info("The usejoblib == True)")
        logging.info(
            "The joblib file for python {} exists.".format(str(python_version))
        )
        logging.info("The mesh will be loaded from {}".format(joblib_file))
        ...
        logging.info("The usejoblib == True")
        logging.info(
            "The joblib file for python {} DO NOT exists".format(str(python_version))
        )
        logging.info("The mesh will be saved to {}".format(joblib_file))
...
"""
    ...
    def __init__(self, path, abg=[50, 15, -90], get3d=True):
        ...
        logging.info(
            "Load 2d part of the grid in {} second(s)".format(str(int(end - start)))
        )
        ...
        if get3d:
            ...
            logging.info(
                "Load 3d part of the grid in {} seconds".format(str(int(end - start)))
            )
...
def read_fesom_mesh(path, alpha, beta, gamma, read_diag=True):
    ...
    with open(aux3dfile) as f:
        ...
        mesh.n32 = (
            np.array([next(f) for x in range(mesh.n2d * mesh.nlev)])
            .astype(int)
            .reshape(mesh.n2d, mesh.nlev)
        )
    ...
    mesh.voltri = abs(np.linalg.det(np.rollaxis(jacobian2D, 2))) / 2.0

    # compute the 2D lump operator
    cnt = np.array((0,) * mesh.n2d)
    mesh.lump2 = np.array((0.0,) * mesh.n2d)
    for i in range(3):
        for j in range(mesh.e2d):
            n = mesh.elem[j, i]
            # cnt[n]=cnt[n]+1
            mesh.lump2[n] = mesh.lump2[n] + mesh.voltri[j]
    mesh.lump2 = mesh.lump2 / 3.0
    ###########################################
    ...
    (mesh.x2, mesh.y2) = ut.scalar_r2g(
        mesh.alpha, mesh.beta, mesh.gamma, mesh.x2, mesh.y2
    )
...
def read_fesom_3d(str_id, months, years, mesh, result_path, runid, ext, how="mean"):
    ...
    while y <= years[1]:
        logging.info(["reading year " + str(y) + ":"])
...
def fesom2depth(depth, data3, mesh, verbose=True):
    ...
    if verbose:
        logging.info(
            "For depth {} model level {} will be used".format(
                str(depth), str(mesh.zlevs[dind])
            )
        )
    return data2
